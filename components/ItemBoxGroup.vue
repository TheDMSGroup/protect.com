...
<template>
  <div class="item_box_group">
    <b-container :class="dynamicClass">
      <b-row>
        <b-col v-if="firstBoxText" :cols="colWidth" class="type_box">
          <item-box :text="firstBoxText" class="nonclick" />
        </b-col>
        <b-col v-for="(option, key) in items" :key="key" :cols="colWidth" class="type_box">
          <item-box :text="option.text" :icon="option.value.icon" :value="option.value.value" />
        </b-col>
      </b-row>
    </b-container>
  </div>
