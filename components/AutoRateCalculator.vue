<script setup>
  ...
  const totalDiscount = computed(() => {
    let discount = 0;
    if (multipleVehicles.value) discount += 0.1;
  ...
  const detectState = () => {
    if (zipCode.value.length >= 3) {
      ...
      const foundEntry = Object.entries(zipToState.value).find(([range]) => {
      ...
      if (foundEntry) {
        ...
        baseRate.value = stateRates.value[foundEntry[1]];
        return;
      }
    }
  ...
  const calculateRate = async () => {
    ...
    if (multipleVehicles.value) multiplier -= 0.1;
    if (hasIncidents.value) multiplier += 0.35;

    if (age.value < 25) multiplier += 0.4;
  ...
  <div class="insurance-calculator-wide">
    ...
    <div class="calculator-layout">
      <transition name="fade-slide" mode="out-in">
        <!-- Left Side: Questions -->
        <div v-if="!showResults" key="questions-panel" class="questions-panel">
        ...
        <!-- Right Side: Results -->
        <div v-else key="results-panel" class="results-panel">
