<template>
  <div class="rate-calculator">
    <b-tabs>
      <b-tab title="Whole Life" @click="toggleTabCalculator" id="whole-life-tab">
        <b-card-text>
          <div v-if="showWholeLife">
            ...
            <form class="tab-form">
              ...
              <div v-if="maritalStatus">
                ...
                <selects :valid="true" :config="maritalStatusConfig" v-model="form.maritalStatus"></selects>
              </div>
              ...
              <div v-if="currentProvider">
                ...
                <selects :valid="true" :config="currentProviderConfig" v-model="form.currentProvider"></selects>
              </div>
      ...
      <b-tab active title="Term Life" @click="toggleTabCalculator" id="term-life-tab">
        <b-card-text>
...
<script setup>
  ...
  // Reactive data
  const showWholeLife = ref(false);
  const showTermLife = ref(true);

  const maritalStatusConfig = ref({
    options: [
      {
        value: {
          value: "single",
  ...
  const currentProviderConfig = ref({
    options: [
      {
        value: {
          value: "geico",
  ...
  const submit = (event) => {
    ...
    router.push({
      path: `/insurance/life-insurance/?zip_code=${form.value.zipCode}&maritalStatus=${form.value.maritalStatus}&gender=${form.value.gender}&currentProvider=${form.value.currentProvider}&desiredPolicyAmount=${form.value.desiredPolicyAmount}`,
    });
  };
  ...
  const toggleTabCalculator = (event) => {
    if (event.currentTarget.id === "term-life-tab___BV_tab_button__") {
      showTermLife.value = true;

      if (showWholeLife.value) {
        showWholeLife.value = false;
      }
    }

    if (event.currentTarget.id === "whole-life-tab___BV_tab_button__") {
      showWholeLife.value = true;

      if (showTermLife.value) {
        showTermLife.value = false;
      }
    }
  };
</script>
