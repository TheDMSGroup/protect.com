<script setup>
  ...
  const getModelDisplayName = (modelSlug) => {
    if (allModelsData.value && Array.isArray(allModelsData.value)) {
      const modelInfo = allModelsData.value.find(
        (m) => m.model_slug === modelSlug
      );
  ...
  const vehicleStats = computed(() => {
    if (modelData.value) {
      return {
        ...
        minimumCoverageAnnual:
          modelData.value["minimum_coverage_annual"] || "620",
      ...
      return [
        {
          ...
          description:
            "Automatically applies brakes to help avoid or reduce the severity of a collision.",
        },
        {
          title: "Lane Keeping Assist",
          description:
            "Helps keep you centered in your lane with gentle steering corrections.",
        },
        {
          title: "Adaptive Cruise Control",
          description:
            "Maintains a set following distance from the vehicle ahead.",
        },
        {
        ...
        {
          title: "Road Departure Mitigation",
          description:
            "Helps prevent unintended road departures with steering and braking assistance.",
        },
      ];
    }
  ...
  const insuranceFactors = computed(() => {
    if (!modelData.value) {
      ...
      return [
        {
          title: "Safety Ratings Impact",
          description:
            "Higher safety ratings typically result in lower insurance premiums due to reduced injury risk in accidents.",
        },
        {
          title: "Repair Cost Considerations",
          description:
            "Parts availability and labor costs for repairs directly affect comprehensive and collision coverage rates.",
        },
        {
          title: "Theft Statistics",
          description:
            "Vehicle theft rates in your area and for this specific model influence comprehensive coverage costs.",
        },
        {
          title: "Driver Demographics",
          description:
            "Your age, driving history, and location significantly impact your personalized insurance rate.",
        },
        {
          title: "Coverage Level Selection",
          description:
            "Choosing higher deductibles or liability-only coverage can substantially reduce your premium.",
        },
      ];
    }
...
<template>
  <div class="vehicle-model-page">
        ...
        <b-row class="hero-row" align-v="center">
          <b-col cols="12" lg="6" class="hero-content">
            ...
            <p class="hero-subtitle">
              Get comprehensive coverage for your {{ formattedMake }}
              {{ formattedModel }}. Compare quotes from top insurers and find
            ...
            <button
              ...
              @click="
                redirectWithParams('https://insure.protect.com', {
                  ...
                  vehicle1model: formattedModel.toUpperCase(),
                })
              "
            >
              Compare Quotes
            ...
            <b-col cols="6" md="3">
              <div class="stat-item">
                ...
                <div class="stat-value">
                  ${{ vehicleStats.fullCoverageAnnual }}
                </div>
              </div>
            </b-col>
            <b-col cols="6" md="3">
              <div class="stat-item">
                ...
                <div class="stat-value">
                  ${{ vehicleStats.minimumCoverageAnnual }}
                </div>
              </div>
            </b-col>
            <b-col cols="6" md="3">
    ...
    <section class="ratings-section">
      <b-container>
        <div class="section-header">
          ...
          <p>
            How the {{ formattedMake }} {{ formattedModel }} performs according
            to industry experts
          </p>
        </div>
        ...
        <b-row class="ratings-grid">
          <b-col cols="12" md="4">
            <div class="rating-card">
              <h3><span>🏆</span> Car & Driver</h3>
              <p>
                {{
                  ratingsData.carDriver ||
                  "Praised for its reliability, fuel efficiency, and overall value in its class."
                }}
              </p>
            </div>
          </b-col>
          <b-col cols="12" md="4">
            <div class="rating-card">
              <h3>
                <span>⭐</span> J.D. Power
                <span v-if="ratingsData.jdPower" class="rating-badge">{{
                  ratingsData.jdPower
                }}</span>
                <span v-else class="rating-badge">82/100</span>
              </h3>
              <p>
                Above average ratings for quality and reliability based on owner
                feedback.
              </p>
            </div>
          </b-col>
          <b-col cols="12" md="4">
            <div class="rating-card">
              <h3><span>🛡️</span> Safety Ratings</h3>
              <p>
                <strong>IIHS:</strong> {{ ratingsData.iihs || "Top Safety Pick"
                }}<br />
    ...
    <section class="factors-section">
      <b-container>
        <div class="section-header">
          ...
          <p>
            What affects your {{ formattedMake }} {{ formattedModel }} insurance
            premium
          </p>
        </div>
        ...
        <div class="factors-list">
          <div
            v-for="factor in insuranceFactors"
            :key="factor.title"
            class="factor-item"
          >
            <h3>{{ factor.title }}</h3>
    ...
    <section v-if="otherModels.length > 0" class="similar-section">
      <b-container>
        <div class="section-header">
          ...
          <p>
            Compare insurance options for other {{ formattedMake }} vehicles
          </p>
        </div>
    ...
    <section class="cta-section">
      <b-container>
        <h2>
          Ready to Save on {{ formattedMake }} {{ formattedModel }} Insurance?
        </h2>
        <p>
          Compare quotes from top insurers and find the best rate for your
          vehicle
        </p>
        <button
          class="cta-button"
    ...
    <div class="rates-disclaimer">
      <b-container>
        <p>
          Rates shown are for a 30-year-old male with full coverage and a clean
          driving record in 2025. Your actual costs will vary based on your
          vehicle's trim, modifications, and driving history.
        </p>
      </b-container>
    </div>
    ...
    <BreadcrumbsMain
      :links="[
        ...
        },
        {
          ...
          url: `/car-insurance/rates-by-vehicle/${make}`,
        },
        { label: `${formattedMake} ${formattedModel} Insurance` },
      ]"
    />
  </div>
</template>
